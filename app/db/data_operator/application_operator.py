...
from app.db.models import Application, ApplicationCreate, ApplicationType
...
class ApplicationOperator:
    ...
    def create(self, app_data: ApplicationCreate) -> Application:
        try:
            ...
            logger.info (f"app_data is {app_data}, and the new_app is {new_app}")
    ...
    def find_by_filter(self, filter_dict: Dict[str, Any], limit: int = 100) -> List[Application]:
        """Find applications by arbitrary filter criteria."""
        try:
            cursor = self.collection.find(filter_dict).limit(limit)
            apps = []
            for app_data in cursor:
                apps.append(Application.from_mongo(app_data))
            logger.info(f"Found {len(apps)} applications matching filter")
            return apps
        except Exception as e:
            logger.error(f"Error finding applications by filter {filter_dict}: {e}")
            return []

    def execute_aggregation(self, pipeline: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Execute aggregation pipeline on applications collection."""
        try:
            results = self.collection.aggregate(pipeline)
            logger.info(f"Aggregation executed on applications collection, returned {len(results)} results")
            return results
        except Exception as e:
            logger.error(f"Error executing aggregation on applications collection: {e}")
            return []

    def execute_query(self, query: Dict[str, Any]) -> Union[List[Application], List[Dict[str, Any]], int]:
        """Execute a MongoDB query generated by LLM on applications collection."""
        try:
            ...
            logger.error(f"Error executing query on applications collection: {e}")
            return []